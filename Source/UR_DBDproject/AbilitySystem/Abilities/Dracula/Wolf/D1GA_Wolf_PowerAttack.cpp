	...
	const FGameplayEventData* TriggerEventData)
{
	...
	Killer->GetCharacterMovement()->MaxWalkSpeed = 300.0f;
	...
	if (KillerController->GetComboIndex() == 2)
	{
		SecondDash();
	}
...
void UD1GA_Wolf_PowerAttack::SecondDash()
{
	UAnimInstance* WolfAnimInstance = Killer->GetMesh()->GetAnimInstance();
...
void UD1GA_Wolf_PowerAttack::EndDash()
{
	...
	if (bSurvivorHit)
	{
		...
		KillerController->SetbCombo(false);
	...
	bool bWasCancelled)
{
	...
	UE_LOG(LogTemp, Log, TEXT("✅ Wolf Power Attack GAS END "));
}